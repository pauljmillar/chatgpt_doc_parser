...
test_file = "20240105-0112431.txt"
#https://www.comperemedia.com/one_search/campaign/20240105-0112431/
...
def test_primary_company(setup_general_1):
    assert "hyundai" in setup_general_1['Primary Company'].lower(), f"Actual: {setup_general_1['Primary Company']}|"
...
def test_vehicle_brand(setup_3):
    assert setup_3['Vehicle Brand'] == 'Hyundai', f"Actual: {setup_3['Vehicle Brand']}|"
...
#    assert "suv" and "crossover" in setup_3['Vehicle Types'].lower(), f"Actual: {setup_3['Vehicle Types']}|"
def test_vehicle_types(setup_3):
...
def test_offer_description(setup_4):
    assert "vehicle" and "apr" in setup_4['Offer Description'].lower(), f"Actual: {setup_4['Offer Description']}|"

def test_event_name(setup_4):
    assert "getaway" and "sales" in setup_4['Event Name'].lower(), f"Actual: {setup_4['Event Name']}|"

def test_vehicle_financing_type(setup_4):
    assert "loan" and "lease" in setup_4['Vehicle Financing Type'].lower(), f"Actual: {setup_4['Vehicle Financing Type']}|"

def test_offer_loan_term(setup_4):
    assert setup_4['Offer Loan Term'] == 60, f"Actual: {setup_4['Offer Loan Term']}|"

def test_offer_apr(setup_4):
    assert setup_4['Offer APR'] == 0, f"Actual: {setup_4['Offer APR']}|"
