...
test_file = "20230408-011999.txt"
#https://www.comperemedia.com/one_search/campaign/20230408-011999/
...
def test_primary_company(setup_general_1):
    assert "toyota" in setup_general_1['Primary Company'].lower(), f"Actual: {setup_general_1['Primary Company']}|"
...
def test_call_to_action(setup_general_2):
    ...
    assert "QR Code to Scan"  or "Customer Visit" in setup_general_2['Response Mechanism'], f"Actual: {setup_general_2['Response Mechanism']}|"
...
def test_vehicle_brand(setup_3):
    assert setup_3['Vehicle Brand'] == 'Toyota', f"Actual: {setup_3['Vehicle Brand']}|"
...
def test_vehicle_types(setup_3):
    ...
    assert "suv" and "compact/economy" and "sedan/midsize" in vehicle_types.lower(), f"Actual: {vehicle_types}|"
...
def test_offer_description(setup_4):
    assert "4.99"  in setup_4['Offer Description'].lower(), f"Actual: {setup_4['Offer Description']}|"

def test_event_name(setup_4):
    assert "automotive sales" in setup_4['Event Name'].lower(), f"Actual: {setup_4['Event Name']}|"
...
def test_offer_apr(setup_4):
    assert setup_4['Offer APR'] == 4.99, f"Actual: {setup_4['Offer APR']}|"

def test_offer_expiration(setup_4):
    assert setup_4['Offer Expiration'] == '2023-05-01', f"Actual: {setup_4['Offer Expiration']}|"
