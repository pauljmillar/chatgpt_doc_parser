...
test_file = "20231109-01157.txt"
#https://www.comperemedia.com/one_search/campaign/20231109-01157/
...
def test_primary_company(setup_general_1):
    assert "jon hall" in setup_general_1['Primary Company'].lower(), f"Actual: {setup_general_1['Primary Company']}|"
...
def test_offer_origin(setup_3):
    assert setup_3['Offer Origin'] == 'Dealer', f"Actual: {setup_3['Offer Origin']}|"

def test_vehicle_brand(setup_3):
    assert setup_3['Vehicle Brand'].lower() == 'chevrolet', f"Actual: {setup_3['Vehicle Brand']}|"
...
def test_offer_description(setup_4):
    assert "trax" or "equinox" or "6.99" in setup_4['Offer Description'].lower(), f"Actual: {setup_4['Offer Description']}|"
...
def test_offer_loan_term(setup_4):
    assert setup_4['Offer Loan Term'] == 84, f"Actual: {setup_4['Offer Loan Term']}|"

def test_offer_apr(setup_4):
    assert setup_4['Offer APR'] == 6.99, f"Actual: {setup_4['Offer APR']}|"
