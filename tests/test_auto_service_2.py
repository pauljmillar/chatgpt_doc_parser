...
def setup_general_1():
    return setup_base('37784235.txt', 'General', 1)

@pytest.fixture(scope="session")
def setup_general_2():
    return setup_base('37784235.txt', 'General', 2)

@pytest.fixture(scope="session")
def setup_general_3():
    return setup_base('37784235.txt', 'Auto-Service', 1)
...
def test_primary_company(setup_general_1):
    assert "Fountain Tire" in setup_general_1['Primary Company']

def test_mailing_type(setup_general_1):
    assert setup_general_1['Mailing Type'] == 'Win Back'
...
def test_presorted(setup_general_1):
    assert setup_general_1['Presorted'] == 'N'

#########################################################
def test_call_to_action(setup_general_2):
    assert "Customer Visit" in setup_general_2['Response Mechanism (Call to Action)'] or "QR Code" in setup_general_2['Response Mechanism (Call to Action)']
    ...
    for incentive in setup_general_2['Incentives']:
        if "Incentive Value" in incentive:
            if incentive['Incentive Value'] == "225":
                assert incentive['Incentive Value'] == "225"
    ...
    for incentive in setup_general_2['Incentives']:
        if "Incentive Text" in incentive:
            if "225" in incentive['Incentive Text']:
                assert "225" in incentive['Incentive Text'] and "tires" in incentive['Incentive Text']
    ...
    for incentive in setup_general_2['Incentives']:
        if "Incentive Value" in incentive:
            if incentive['Incentive Value'] == "50":
                assert incentive['Incentive Value'] == "50"
    ...
    for incentive in setup_general_2['Incentives']:
        if "Incentive Text" in incentive:
            if "50" in incentive['Incentive Text']:
                assert "50" in incentive['Incentive Text'] and "service" in incentive['Incentive Text']
...
def test_product(setup_general_3):
    assert "service" in setup_general_3['Product'].lower()

def test_offer_summary(setup_general_3):
    assert "save" or "discount" in setup_general_3['Offer Summary'].lower() and "tires" or "service" in setup_general_3['Offer Summary'].lower()

def test_offer_close_date(setup_general_3):
    assert setup_general_3['Offer Close Date'] == 'December 16, 2023'
